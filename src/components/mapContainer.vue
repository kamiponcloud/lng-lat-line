<template>
  <div class="map-container">
    <div id="map"></div>
    <div class="open-btn btn" @click="openMiniMap">打开经纬度线</div>
    <div class="close-btn btn" @click="closeMniMap">关闭经纬度线</div>
  </div>
</template>
...
import LngLatLine from '../utils/lng-lat-line.js'
export default {
  name: 'mapContainer',
  props: {
    msg: String
  ...
  methods: {
    openMiniMap () {
      if (!this.miniMap) {
        ...
        this.miniMap.addLineToMap()
      }
    },
    closeMniMap () {
      if (this.miniMap) {
        this.miniMap.removeLineToMap()
